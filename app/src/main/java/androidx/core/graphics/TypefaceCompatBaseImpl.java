...
class TypefaceCompatBaseImpl {
    ...
    private static <T> T findBestFont(T[] fonts, int style, StyleExtractor<T> extractor) {
        int targetWeight = (style & 1) == 0 ? HttpStatus.SC_BAD_REQUEST : TypedValues.Transition.TYPE_DURATION;
    ...
    public Typeface createFromFontFamilyFilesResourceEntry(Context context, FontResourcesParserCompat.FontFamilyFilesResourceEntry entry, Resources resources, int style) throws NoSuchFieldException {
    ...
    private void addFontFamily(Typeface typeface, FontResourcesParserCompat.FontFamilyFilesResourceEntry entry) throws NoSuchFieldException {
        long key = getUniqueKey(typeface);
        if (key != 0) {
