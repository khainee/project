...
public class NotificationCompat {
    ...
    public static class Builder {
        ...
        private Bitmap reduceLargeIconSize(Bitmap icon) throws Resources.NotFoundException {
            ...
            double d = maxWidth;
            double dMax = Math.max(1, icon.getWidth());
            Double.isNaN(d);
            Double.isNaN(dMax);
            double d2 = d / dMax;
            double d3 = maxHeight;
            double dMax2 = Math.max(1, icon.getHeight());
            Double.isNaN(d3);
            Double.isNaN(dMax2);
            double scale = Math.min(d2, d3 / dMax2);
            double width = icon.getWidth();
            Double.isNaN(width);
            int iCeil = (int) Math.ceil(width * scale);
            double height = icon.getHeight();
            Double.isNaN(height);
            return Bitmap.createScaledBitmap(icon, iCeil, (int) Math.ceil(height * scale), true);
        }
    ...
    public static abstract class Style {
        ...
        private Bitmap createColoredBitmap(IconCompat icon, int color, int size) throws IllegalAccessException, NoSuchMethodException, PackageManager.NameNotFoundException, SecurityException, IllegalArgumentException, InvocationTargetException {
    ...
    public static class MessagingStyle extends Style {
        ...
        private CharSequence makeMessageLine(Message message) {
            ...
            int color2 = afterLollipop ? ViewCompat.MEASURED_STATE_MASK : -1;
