...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        for (int i = 0; i < iMin; i++) {
            pathMeasure.getPosTan((i * length) / (iMin - 1), fArr, null);
            this.mX[i] = fArr[0];
            this.mY[i] = fArr[1];
        }
        if (Math.abs(this.mX[0]) > 1.0E-5d || Math.abs(this.mY[0]) > 1.0E-5d || Math.abs(this.mX[iMin - 1] - 1.0f) > 1.0E-5d || Math.abs(this.mY[iMin - 1] - 1.0f) > 1.0E-5d) {
            throw new IllegalArgumentException("The Path must start at (0,0) and end at (1,1) start: " + this.mX[0] + "," + this.mY[0] + " end:" + this.mX[iMin - 1] + "," + this.mY[iMin - 1]);
        }
        float f = 0.0f;
        int i2 = 0;
        int i3 = 0;
