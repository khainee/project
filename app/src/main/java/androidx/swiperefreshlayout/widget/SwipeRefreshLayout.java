...
public class SwipeRefreshLayout extends ViewGroup implements NestedScrollingParent3, NestedScrollingParent2, NestedScrollingChild3, NestedScrollingChild2, NestedScrollingParent, NestedScrollingChild {
    ...
    private void moveSpinner(float f) {
        ...
        float f3 = f2;
        float fMax2 = Math.max(0.0f, Math.min(fAbs, f3 * DECELERATE_INTERPOLATION_FACTOR) / f3);
        float fPow = ((float) ((fMax2 / 4.0f) - Math.pow(fMax2 / 4.0f, 2.0d))) * DECELERATE_INTERPOLATION_FACTOR;
        int i = this.mOriginalOffsetTop + ((int) ((f3 * fMin) + (f3 * fPow * DECELERATE_INTERPOLATION_FACTOR)));
        ...
        this.mProgress.setProgressRotation(((-0.25f) + (0.4f * fMax) + (fPow * DECELERATE_INTERPOLATION_FACTOR)) * 0.5f);
        setTargetOffsetTopAndBottom(i - this.mCurrentTargetOffsetTop);
    }
