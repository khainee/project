...
public class Debug {
        ...
        for (Field declaredField : declaredFields) {
            try {
                ...
                if (declaredField.getName().startsWith("layout_constraint") && ((!(value instanceof Integer) || !value.toString().equals("-1")) && ((!(value instanceof Integer) || !value.toString().equals("0")) && ((!(value instanceof Float) || !value.toString().equals("1.0")) && (!(value instanceof Float) || !value.toString().equals("0.5")))))) {
                    ...
                    StringBuilder sb3 = new StringBuilder(String.valueOf(loc).length() + 5 + String.valueOf(name2).length() + String.valueOf(strValueOf).length());
                    sb3.append(loc);
                    sb3.append("    ");
                    sb3.append(name2);
                    sb3.append(" ");
    ...
    public static String getName(Context context, int[] id) throws Resources.NotFoundException {
        ...
        try {
            ...
            while (i < id.length) {
                ...
                String strValueOf2 = String.valueOf(i == 0 ? "" : " ");
                String str2 = strValueOf2.length() != 0 ? strValueOf.concat(strValueOf2) : new String(strValueOf);
                try {
                ...
                String strValueOf4 = String.valueOf(tmp);
                str = strValueOf4.length() != 0 ? strValueOf3.concat(strValueOf4) : new String(strValueOf3);
                i++;
            }
            ...
            for (Field declaredField : declaredFields) {
                try {
                    ...
                    if (name2.contains("To") && !value.toString().equals("-1")) {
                        ...
                        String strValueOf = String.valueOf(value);
                        StringBuilder sb4 = new StringBuilder(String.valueOf(loc).length() + 8 + String.valueOf(name3).length() + String.valueOf(strValueOf).length());
                        sb4.append(loc);
                        sb4.append("       ");
                        sb4.append(name3);
                        sb4.append(" ");
    ...
    public static void dumpLayoutParams(ViewGroup.LayoutParams param, String str) throws IllegalAccessException, SecurityException, IllegalArgumentException {
        ...
        for (Field declaredField : declaredFields) {
            try {
                ...
                if (name2.contains("To") && !value.toString().equals("-1")) {
                    PrintStream printStream2 = System.out;
                    String strValueOf = String.valueOf(value);
                    StringBuilder sb3 = new StringBuilder(String.valueOf(loc).length() + 8 + String.valueOf(name2).length() + String.valueOf(strValueOf).length());
                    sb3.append(loc);
                    sb3.append("       ");
                    sb3.append(name2);
                    sb3.append(" ");
        ...
        PrintStream printStream3 = System.out;
        String strValueOf2 = String.valueOf(loc);
        printStream3.println(strValueOf2.length() != 0 ? " <<<<<<<<<<<<<<<<< dump ".concat(strValueOf2) : new String(" <<<<<<<<<<<<<<<<< dump "));
    }
}
