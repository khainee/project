    ...
    private float getPreCycleDistance() {
        ...
        while (i < pointCount) {
            ...
            if (i2 > 0) {
                ...
                double d4 = points[0];
                Double.isNaN(d4);
                double dHypot = Math.hypot(y - d3, x - d4);
                Double.isNaN(d2);
                sum = (float) (d2 + dHypot);
            }
            x = points[0];
    ...
    public void setup(int parentWidth, int parentHeight, float transitionDuration, long currentTime) {
        ...
        for (String s : this.mEndMotionPath.attributes.keySet()) {
            if (this.mStartMotionPath.attributes.containsKey(s)) {
                ...
                if (!splineAttributes.contains(strValueOf.length() != 0 ? "CUSTOM,".concat(strValueOf) : new String("CUSTOM,"))) {
        ...
        while (true) {
            ...
            if (interpolateVariable < MotionPaths.names.length) {
                ...
                while (true) {
                    ...
                    if (i7 < points.length) {
                        String strValueOf2 = String.valueOf(s2);
    ...
    */
    boolean interpolate(View child, float global_position, long time, KeyCache keyCache) {
        ...
        if (map2 == null) {
            ...
            Iterator<ViewTimeCycle> it2 = map2.values().iterator();
        ...
        if (curveFitArr != null) {
            ...
            if (map3 != null) {
                for (SplineSet aSpline2 : map3.values()) {
                    if (aSpline2 instanceof ViewSpline.PathRotate) {
                        double[] dArr2 = this.mInterpolateVelocity;
                        if (dArr2.length > 1) {
                            ((ViewSpline.PathRotate) aSpline2).setPathRotate(child, position, dArr2[0], dArr2[1]);
                        }
                    }
                }
            }
        ...
        if (map4 != null) {
            for (ViewOscillator osc : map4.values()) {
