    ...
    private void viewTransition(ViewTransition vt, View... view) {
        ...
        if (vt.mViewTransitionMode != 2) {
            if (currentId == -1) {
                ...
                Log.w(str, strValueOf.length() != 0 ? "No support for ViewTransition within transition yet. Currently: ".concat(strValueOf) : new String("No support for ViewTransition within transition yet. Currently: "));
                return;
            } else {
