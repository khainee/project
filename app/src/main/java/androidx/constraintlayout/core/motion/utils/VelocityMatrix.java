...
public class VelocityMatrix {
    ...
    public void applyTransform(float locationX, float locationY, int width, int height, float[] mAnchorDpDt) {
        ...
        double d = (-width) * offx;
        double dSin = Math.sin(r);
        Double.isNaN(d);
        double d2 = d * dSin;
        double d3 = height * offy;
        double dCos = Math.cos(r);
        Double.isNaN(d3);
        float dx4 = dx3 + (((float) (d2 - (d3 * dCos))) * dr);
        double d4 = width * offx;
        double dCos2 = Math.cos(r);
        Double.isNaN(d4);
        double d5 = d4 * dCos2;
        double d6 = height * offy;
        double dSin2 = Math.sin(r);
        Double.isNaN(d6);
        mAnchorDpDt[0] = dx4;
        mAnchorDpDt[1] = dy3 + (((float) (d5 - (d6 * dSin2))) * dr);
    }
}
