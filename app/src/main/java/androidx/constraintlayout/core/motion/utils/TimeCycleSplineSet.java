...
public abstract class TimeCycleSplineSet {
    ...
    public static class CustomSet extends TimeCycleSplineSet {
        ...
        public boolean setProperty(MotionWidget view, float t, long time, KeyCache cache) throws IllegalAccessException, NoSuchMethodException, SecurityException, IllegalArgumentException, InvocationTargetException {
        ...
        for (int i3 = 0; i3 < this.count; i3++) {
            if (i3 > 0) {
                ...
                if (iArr2[i3] != iArr2[i3 - 1]) {
                    double d = this.mTimePoints[i3];
                    Double.isNaN(d);
                    time[k] = d * 0.01d;
                    double[] dArr = values[k];
                    float[][] fArr = this.mValues;
                    dArr[0] = fArr[i3][0];
                    values[k][1] = fArr[i3][1];
                    values[k][2] = fArr[i3][2];
                    k++;
                }
            }
        }
