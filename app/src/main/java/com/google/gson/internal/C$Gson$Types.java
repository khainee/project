...
public final class C$Gson$Types {
    ...
    private static Type resolve(Type type, Class<?> cls, Type type2, Collection<TypeVariable> collection) {
        ...
        if (type2 instanceof GenericArrayType) {
            GenericArrayType genericArrayTypeArrayOf = (GenericArrayType) type2;
            Type genericComponentType = genericArrayTypeArrayOf.getGenericComponentType();
            Type typeResolve2 = resolve(type, cls, genericComponentType, collection);
            if (genericComponentType != typeResolve2) {
                genericArrayTypeArrayOf = arrayOf(typeResolve2);
            }
            return genericArrayTypeArrayOf;
        }
        if (type2 instanceof ParameterizedType) {
            ParameterizedType parameterizedTypeNewParameterizedTypeWithOwner = (ParameterizedType) type2;
            Type ownerType = parameterizedTypeNewParameterizedTypeWithOwner.getOwnerType();
            Type typeResolve3 = resolve(type, cls, ownerType, collection);
            boolean z = typeResolve3 != ownerType;
            Type[] actualTypeArguments = parameterizedTypeNewParameterizedTypeWithOwner.getActualTypeArguments();
            ...
            if (z) {
                parameterizedTypeNewParameterizedTypeWithOwner = newParameterizedTypeWithOwner(typeResolve3, parameterizedTypeNewParameterizedTypeWithOwner.getRawType(), actualTypeArguments);
            }
