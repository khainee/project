...
public final class Cookie {
    ...
    static Cookie parse(long currentTimeMillis, HttpUrl url, String setCookie) {
        ...
        while (pos < limit) {
            ...
            } else if (attributeName.equalsIgnoreCase(ClientCookie.DOMAIN_ATTR)) {
                try {
                    domain = parseDomain(attributeValue);
                    hostOnly = false;
                } catch (IllegalArgumentException e3) {
                }
            } else if (attributeName.equalsIgnoreCase(ClientCookie.PATH_ATTR)) {
                path = attributeValue;
            } else if (attributeName.equalsIgnoreCase(ClientCookie.SECURE_ATTR)) {
                secureOnly = true;
        ...
        if (deltaSeconds == Long.MIN_VALUE) {
            ...
            if (deltaSeconds <= 9223372036854775L) {
                ...
                deltaMilliseconds = LocationRequestCompat.PASSIVE_INTERVAL;
            }
    ...
    private static long parseExpires(String s, int pos, int limit) throws NumberFormatException {
    ...
    private static long parseMaxAge(String s) throws NumberFormatException {
        try {
            ...
            if (s.matches("-?\\d+")) {
                return s.startsWith("-") ? Long.MIN_VALUE : LocationRequestCompat.PASSIVE_INTERVAL;
            }
            throw e;
        }
    }
